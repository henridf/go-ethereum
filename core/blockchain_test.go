...
func TestFastVsFullChains(t *testing.T) {
	...
	ancientDb, err := rawdb.NewDatabaseWithFreezer(rawdb.NewMemoryDatabase(), frdir, 0, "", false)
	if err != nil {
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	makeDb := func() (ethdb.Database, func()) {
		...
		defer os.Remove(dir)
		db, err := rawdb.NewDatabaseWithFreezer(rawdb.NewMemoryDatabase(), dir, 0, "", false)
		if err != nil {
			t.Fatalf("failed to create temp freezer db: %v", err)
		}
		gspec.MustCommit(db)
...
func testInsertKnownChainData(t *testing.T, typ string) {
	...
	defer os.Remove(dir)
	chaindb, err := rawdb.NewDatabaseWithFreezer(rawdb.NewMemoryDatabase(), dir, 0, "", false)
	if err != nil {
		t.Fatalf("failed to create temp freezer db: %v", err)
	}
