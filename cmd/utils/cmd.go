...
func GapfillChain(chain *core.BlockChain, tc *params.TrustedCheckpoint, fn string) error {
	...
	for batch := 0; ; batch++ {
		...
		log.Info("window info", "wstart", batch*importBatchSize*2, "wend", wend)
		...
		log.Info("missing info", "len(blocks)", len(blocks), "first", blocks[0].Number(), "last", blocks[len(blocks)-1].Number(), "len(missingBlocks)", len(missingBlocks))
